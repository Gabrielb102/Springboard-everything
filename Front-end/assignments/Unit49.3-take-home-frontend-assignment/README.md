...
At first I was overcomplicating this assignment. I tried to add a whole new React App that would allow me to use React Components and would require that I interact with the backend as if it were a server on another machine. I also tried to create a whole other file that ran with it's own frontend and would have to be launched separately. It wasn't until about 5 or 6 hours into these attempts before I realized I just needed to write the JavaScript files to make these pages function, then add some styling to make it work. As a result, I only spent about 4 hours on the completed project you see here, and about 10 hours overall. 

The app could definitely use some polishing, especially the forms. I spruced up the homepage and the admin page by adding some nice button styling and background color and borders for each user listing. For the admin page, I also added the functionality to go to an edit user page for each user, and a remove button to delete them. Both the edit and registration forms are unstyled, however they are both functional, and edit the user "database" as they should.
