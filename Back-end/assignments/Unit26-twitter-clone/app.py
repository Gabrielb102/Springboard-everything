...
def show_likes(user_id):
    """Display a list of a user's liked messages"""
    user = User.query.get_or_404(user_id)
    likes = user.likes
    raise
...
def add_like(message_id):
    """adds a like to the message from a certain user"""
    user = g.user
    if user: 
        ...
        flash("Must be logged in to like warbles")
...
def un_like(message_id):
    ...
    if user: 
        message = Message.query.get_or_404(message_id)
        i_to_unlike = user.likes.index(message)
        user.likes.pop(i_to_unlike)
        db.session.add(user)
        db.session.commit()
        return redirect('/')
    else: 
        flash("Must be logged in to unlike warbles")
        return redirect('/login')
