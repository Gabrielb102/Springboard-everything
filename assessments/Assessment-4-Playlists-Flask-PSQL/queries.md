...
```sql
SELECT title, name AS studio FROM movies JOIN studios ON studios.id = movies.studio_id;
```
...
```sql
SELECT DISTINCT first_name, last_name, count(*) AS number_of_movies FROM roles JOIN stars ON roles.star_id = stars.id GROUP BY stars.id ORDER BY number_of_movies DESC;
```
